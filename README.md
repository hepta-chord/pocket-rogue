...
- アイテムは踏むと拾う。武器 `)` と防具 `[` は今より強ければ自動で持ち替える。
  武器は階が深いほど強く (`1 + 階/3 + 0〜1`)、防具は伸びが鈍い (`1 + 階/4`)
...
| 敵 | 系統 | 階 | 防御 | パッシブ | アクション |
|---|---|---|---|---|---|
| ネズミ `r` | 群れ | B1〜B4 | 0 | なし (2〜3 匹の群れ) | なし |
| コウモリ `b` | 俊敏 | B1〜B6 | 0 | 俊敏 (1 ターン 2 回行動)、ふらつき | なし |
| ゴブリン `g` | 戦士 | B2〜B8 | 1 | なし | 連撃 30%: 2 回攻撃 |
| スライム `s` | 変則 | B3〜B7 | 1 | 分裂: 近接攻撃を受けて生き残ると 2 匹に (4 匹まで) | なし |
| オーク `o` | 戦士 | B4〜 | 2 | なし | なし |
| 幽霊 `G` | 変則 | B5〜 | 0 | 壁抜け | なし |
| トロル `T` | 重装 | B6〜 | 3 | 再生 (毎ターン HP 1)、鈍重 (2 ターンに 1 回) | 強打 25%: 防具を無視 |
| 狼 `w` | 俊敏 | B7〜 | 1 | 俊敏 | 跳びかかり 40%: 距離 2 から一気に隣接して攻撃 |
| ドラゴン `D` | ボス | B9〜 (1 体) | 3 | なし | 火炎 30%: 距離 2〜4 で炎 (攻撃力の半分 + 階数、防具で軽減) |

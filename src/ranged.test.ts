...
describe('槍 (射程 2 の直線)', () => {
  ...
  it('通路の脇からでも、斜めの射線が通っていれば届く', () => {
    // 部屋の (1,1) から通路口の (3,3) へ抜ける斜め。
    // 途中の (2,2) は床だが、(2,3) は壁である。
    // 移動と同じ厳しさにすると (2,3) が壁なので通らず、撃ってこなくなる
    const s = board('SPEAR4', ['#####', '#M..#', '#...#', '###P#', '###.#']);
    s.player.x = 3;
    s.player.y = 3;
    put(s, 'koboldSpear', 1, 1, 100);
    expect(hitWithin(s, 8)).toBe(true);
  });

  it('壁の角が点で触れているだけの隙間は通らない', () => {
    // (1,1) と (2,2) は斜めに隣り合うが、(2,1) も (1,2) も壁である。
    // ここまで通すと射線が事実上どこへでも届いてしまう
    const s = board('SPEAR5', ['#####', '#M###', '##..#', '#..P#', '#####']);
    s.player.x = 3;
    s.player.y = 3;
    put(s, 'koboldSpear', 1, 1, 100);
    // 詰めてくることもできない配置なので、何ターン経っても届かない
    expect(hitWithin(s, 20)).toBe(false);
  });
